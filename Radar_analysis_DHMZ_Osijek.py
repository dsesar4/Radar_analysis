...
DHMZ_possible_reflections = [[0,255,255],[0,199,199],[0,146,145],[0,56,184],\
                             ...
                             [237,194,0],[255,0,0],[247,0,219],[161,0,161]]
df3 = pd.DataFrame(DHMZ_possible_reflections, columns = ['r','g','b'])

print('\nWelcome to DHMZ-Osijek radar analysis! This analysis is suitable only for periods after'\
      ' 6.6.2019. Please enter following parameters:')
a = int(input('Area for analysis (0-Slavonia, 1-East Slavonia, 2-Posavina): '))
year = int(input('Year: '))
...
areas = [[100,180,310,320],[180,180,320,320],[100,250,300,320]]
...
while (k <= iterations):
    ...
    urllib.request.urlretrieve('http://139.59.144.6/arhiva/osijek_stat/{year}/{month}/{day}/osijek_stat_{year}{month}{day}{hour}{minute}Z.png'\
                               .format(year = year, month = month_querry, day = day_querry, hour = hour_querry, minute = minute_querry), "osijek-stat.png")
    
    im = Image.open("osijek-stat.png")
    ...
    for i in range (areas[a][0],areas[a][2]):
        for j in range (areas[a][1],areas[a][3]):
            ...
            arr.append([r,g,b])
            if ((r == df3['r'][1] or r == df3['r'][2] or r == df3['r'][3] or r == df3['r'][4]) and (g == df3['g'][1] or g == df3['g'][2] or g == df3['g'][3] or g == df3['g'][4])):
                x_rain[j - areas[a][1]][i - areas[a][0]] = x_rain[j - areas[a][1]][i - areas[a][0]] + 1
            elif ((r == df3['r'][5] or r == df3['r'][6]) and (g == df3['g'][5] or g == df3['g'][6])):
                x_shower[j - areas[a][1]][i - areas[a][0]] = x_shower[j - areas[a][1]][i - areas[a][0]] + 1
            elif ((r == df3['r'][7] or r == df3['r'][8]) and (g == df3['g'][7] or g == df3['g'][8])):
                x_thunderstorm[j - areas[a][1]][i - areas[a][0]] = x_thunderstorm[j - areas[a][1]][i - areas[a][0]] + 1
            elif ((r == df3['r'][9] or r == df3['r'][10] or r == df3['r'][11]) and (g == df3['g'][9] or g == df3['g'][10] or g == df3['g'][11])):
...
template = Image.open("template_osijek.png")
